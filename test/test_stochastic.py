...
class TestLogBatchMarginal(TestCase):
    def test_normal(self):
        ...
        log_joint_2[range(B), range(B)] += math.log(B/(N-1))
        log_joint_2 = log_mean_exp(log_joint_2, 1).transpose(0, 1) +  math.log((N-1)/N)

        log_sum_1[range(B), range(B)] += math.log(B/(N-1))
        log_sum_2[range(B), range(B)] += math.log(B/(N-1))
        log_mar_z1 = log_mean_exp(log_sum_1, 1).transpose(0, 1) + math.log((N-1)/N)
        log_mar_z2 = log_mean_exp(log_sum_2, 1).transpose(0, 1) +  math.log((N-1)/N)
        log_mar_2 = log_mar_z1 + log_mar_z2

        log_probs1[range(B), range(B)] += math.log(B/(N-1))
        log_probs2[range(B), range(B)] += math.log(B/(N-1))
        log_prod_mar_z1 = (log_mean_exp(log_probs1, 1)+  math.log((N-1)/N)).sum(2).transpose(0, 1)
        log_prod_mar_z2 = (log_mean_exp(log_probs2, 1)+  math.log((N-1)/N)).sum(2).transpose(0, 1)
